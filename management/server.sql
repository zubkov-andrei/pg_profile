...
BEGIN
  IF has_column_privilege('servers', 'connstr', 'SELECT') THEN
    FOR server_name, server_hostname, server_ip, server_port, cpu_cores, cpu_model, cpu_mhz, cpu_sockets, memory_total_mb, connstr, enabled, max_sample_age, description IN
      SELECT s.server_name, s.server_hostname, s.server_ip, s.server_port, s.cpu_cores, s.cpu_model, s.cpu_mhz, s.cpu_sockets, s.memory_total_mb, s.connstr, s.enabled, s.max_sample_age, s.server_description
      FROM servers s
  ...
  ELSE
    FOR server_name, server_hostname, server_ip, server_port, cpu_cores, cpu_model, cpu_mhz, cpu_sockets, memory_total_mb, connstr, enabled, max_sample_age, description IN
      SELECT s.server_name, s.server_hostname, s.server_ip, s.server_port, s.cpu_cores, s.cpu_model, s.cpu_mhz, s.cpu_sockets, s.memory_total_mb, '<hidden>' as connstr, s.enabled, s.max_sample_age, s.server_description
      FROM servers s
      WHERE s.server_hostname = hostname_filter OR hostname_filter IS NULL
    LOOP
      RETURN NEXT;
