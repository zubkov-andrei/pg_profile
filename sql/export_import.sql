...
SELECT
  CASE
    WHEN
      md5(profile.get_report('local',1,3)) !=
      md5(replace(profile.get_report('src_local',1,3),'<p>Server name: <strong>src_local</strong></p>',
        '<p>Server name: <strong>local</strong></p>'))
    THEN
      format(E'<imported_start>\n%s\n<imported_end>\n<exported_start>\n%s\n<exported_end>',profile.get_report('local',1,3),
        replace(profile.get_report('src_local',1,3),'<p>Server name: <strong>src_local</strong></p>',
        '<p>Server name: <strong>local</strong></p>')
      )
    ELSE
      'ok'
