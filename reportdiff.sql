...
BEGIN
    ...
    'report_features',jsonb_build_object(
      'statstatements',profile_checkavail_statstatements(sserver_id, start1_id, end1_id) OR profile_checkavail_statstatements(sserver_id, start2_id, end2_id),
      'statstatements_v1.8',profile_checkavail_statements_v18(sserver_id, start1_id, end1_id) OR profile_checkavail_statements_v18(sserver_id, start2_id, end2_id),
      'kcachestatements',profile_checkavail_kcachestatements(sserver_id, start1_id, end1_id) OR profile_checkavail_kcachestatements(sserver_id, start2_id, end2_id)
    ));
...
COMMENT ON FUNCTION get_diffreport(IN sserver_id integer, IN start1_id integer, IN end1_id integer, IN start2_id integer,
...
$$ LANGUAGE sql;
COMMENT ON FUNCTION get_diffreport(IN server name, IN start1_id integer, IN end1_id integer, IN start2_id integer,
IN end2_id integer, IN description text)
...
COMMENT ON FUNCTION get_diffreport(IN start1_id integer, IN end1_id integer, IN start2_id integer,
IN end2_id integer, IN description text)
...
$$ LANGUAGE sql;
COMMENT ON FUNCTION get_diffreport(IN server name, IN baseline1 varchar(25), IN baseline2 varchar(25),
IN description text)
...
$$ LANGUAGE sql;
COMMENT ON FUNCTION get_diffreport(IN server name, IN baseline varchar(25), IN start2_id integer,
IN end2_id integer, IN description text)
...
RETURNS text SET search_path=@extschema@,public AS $$
  SELECT get_diffreport(get_server_by_name(server),tm1.start_id,tm1.end_id,tm2.start_id,tm2.end_id,description)
...
$$ LANGUAGE sql;
COMMENT ON FUNCTION get_diffreport(IN server name, IN time_range1 tstzrange, IN time_range2 tstzrange,
IN description text)
...
CREATE FUNCTION get_diffreport(IN server name, IN baseline varchar(25),
  ...
  IN description text = NULL)
RETURNS text SET search_path=@extschema@,public AS $$
  SELECT get_diffreport(get_server_by_name(server),bl1.start_id,bl1.end_id,tm2.start_id,tm2.end_id,description)
  FROM get_baseline_samples(get_server_by_name(server), baseline) bl1
    CROSS JOIN get_sampleids_by_timerange(get_server_by_name(server), time_range) tm2
$$ LANGUAGE sql;
COMMENT ON FUNCTION get_diffreport(IN server name, IN baseline varchar(25), IN time_range tstzrange,
IN description text)
...
CREATE FUNCTION get_diffreport(IN server name, IN time_range tstzrange,
  ...
  IN description text = NULL)
RETURNS text SET search_path=@extschema@,public AS $$
  SELECT get_diffreport(get_server_by_name(server),tm1.start_id,tm1.end_id,bl2.start_id,bl2.end_id,description)
  FROM get_baseline_samples(get_server_by_name(server), baseline) bl2
    CROSS JOIN get_sampleids_by_timerange(get_server_by_name(server), time_range) tm1
$$ LANGUAGE sql;
COMMENT ON FUNCTION get_diffreport(IN server name, IN time_range tstzrange, IN baseline varchar(25),
IN description text)
