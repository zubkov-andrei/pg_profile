...
ALTER TABLE snap_stat_user_tables_old RENAME CONSTRAINT pk_snap_stat_user_tables TO pk_snap_stat_user_tables_old;
...
INSERT INTO last_stat_user_tables SELECT * FROM ONLY last_stat_user_tables_old;
DROP TABLE last_stat_user_tables_old;

INSERT INTO tables_list
    SELECT DISTINCT relid, schemaname, relname FROM ONLY snap_stat_user_tables_old
...
INSERT INTO snap_stat_user_tables
    SELECT
        ...
        relsize,
        relsize_diff
    FROM ONLY snap_stat_user_tables_old;

DROP TABLE snap_stat_user_tables_old;
...
ALTER TABLE snap_stat_user_indexes_old RENAME CONSTRAINT pk_snap_stat_user_indexes TO pk_snap_stat_user_indexes_old;
...
INSERT INTO indexes_list
    SELECT DISTINCT indexrelid, schemaname, indexrelname
    FROM ONLY snap_stat_user_indexes_old
    ON CONFLICT DO NOTHING;
...
INSERT INTO last_stat_user_indexes
    SELECT * FROM ONLY last_stat_user_indexes_old;
DROP TABLE last_stat_user_indexes_old;
...
DROP TABLE snap_stat_user_indexes_old;
...
ALTER TABLE snap_stat_user_functions_old RENAME CONSTRAINT pk_snap_stat_user_functions TO pk_snap_stat_user_functions_old;
...
INSERT INTO funcs_list 
    SELECT DISTINCT funcid, schemaname, funcname
    FROM ONLY snap_stat_user_functions_old;

INSERT INTO last_stat_user_functions SELECT * FROM ONLY last_stat_user_functions_old;
DROP TABLE last_stat_user_functions_old;
...
INSERT INTO snap_stat_user_functions
    SELECT
        ...
        self_time
    FROM ONLY snap_stat_user_functions_old;
DROP TABLE snap_stat_user_functions_old;

ALTER TABLE snap_statio_user_tables RENAME TO snap_statio_user_tables_old;
ALTER TABLE snap_statio_user_tables_old RENAME CONSTRAINT pk_snap_statio_user_tables TO pk_snap_statio_user_tables_old;
...
INSERT INTO last_statio_user_tables SELECT * FROM last_statio_user_tables_old;
DROP TABLE last_statio_user_tables_old;

INSERT INTO tables_list
    SELECT DISTINCT relid, schemaname, relname 
    FROM ONLY snap_statio_user_tables_old
    ON CONFLICT DO NOTHING;
...
ALTER TABLE snap_statio_user_indexes_old RENAME CONSTRAINT pk_snap_statio_user_indexes TO pk_snap_statio_user_indexes_old;
...
INSERT INTO last_statio_user_indexes SELECT * FROM ONLY last_statio_user_indexes_old;
DROP TABLE last_statio_user_indexes_old;

INSERT INTO tables_list
    SELECT DISTINCT relid, schemaname, relname 
    FROM ONLY snap_statio_user_indexes_old
    ON CONFLICT DO NOTHING;
...
INSERT INTO snap_statio_user_indexes
    SELECT 
        ...
        relsize_diff
    FROM ONLY snap_statio_user_indexes_old;
DROP TABLE snap_statio_user_indexes_old;

ALTER TABLE snap_stat_database ADD COLUMN datsize_delta bigint;
...
INSERT INTO last_stat_database SELECT * FROM ONLY last_stat_database_old;

UPDATE last_stat_database SET datsize_delta = 0;
UPDATE snap_stat_database SET datsize_delta = 0;

DROP TABLE last_stat_database_old;
